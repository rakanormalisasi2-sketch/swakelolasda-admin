...
export default function SeksiEmbungMapPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const { data: assignments, error } = await supabase
        ...
        `)
        .eq('status', 'active')
        .eq('created_by_role', 'seksi_embung')
      ...
      console.error('Error loading seksi embung map:', err);
    } finally {
      setLoading(false);
    }
  }, [buildMapItems]);
  ...
  return (
    <>
      <div className="header">
        <div>
          <div className="header-title">Peta Sebaran — Seksi Embung</div>
          <div className="header-subtitle">Lokasi penugasan aktif seksi Embung</div>
        </div>
      ...
      <div style={{
        ...
        {loading ? (
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%', color: 'var(--text-muted)' }}>
            Tidak ada penugasan aktif seksi Embung.
          </div>
        ) : (
