...
export default function PeralatanPage() {
  ...
  // Warehouse Summary States
  const [warehouseSummary, setWarehouseSummary] = useState({ pendingRequests: 0, lowStockItems: 0 });
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const [whReq, whItems] = await Promise.all([
        fetch('/api/gudang?type=requests').then(r => r.json()),
        fetch('/api/gudang?type=items').then(r => r.json())
      ]);
      const pending = (whReq.data || []).filter(r => r.status === 'pending').length;
      const lowStock = (whItems.data || []).filter(i => i.current_stock <= i.min_stock).length;
      setWarehouseSummary({ pendingRequests: pending, lowStockItems: lowStock });
    } catch (err) {
      ...
      <div className="page-body">
        ...
        {mainTab === 'inventaris' && (
           <>
            ...
            {(warehouseSummary.pendingRequests > 0 || warehouseSummary.lowStockItems > 0) && (
              <div style={{ display: 'flex', gap: 15, marginBottom: 20 }}>
                {warehouseSummary.pendingRequests > 0 && (
                  <div style={{ flex: 1, background: '#eff6ff', border: '1px solid #bfdbfe', borderRadius: 12, padding: 15, display: 'flex', alignItems: 'center', gap: 12, cursor: 'pointer' }} onClick={() => router.push('/dashboard/peralatan/gudang')}>
                    <div style={{ background: '#3b82f6', color: '#fff', width: 40, height: 40, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20 }}>📦</div>
                    <div>
                      <div style={{ fontWeight: 'bold', color: '#1e3a8a' }}>{warehouseSummary.pendingRequests} Permintaan Barang</div>
                      <div style={{ fontSize: 12, color: '#60a5fa' }}>Menunggu persetujuan Anda</div>
                    </div>
                    <div style={{ marginLeft: 'auto', color: '#3b82f6', fontWeight: 'bold' }}>Lihat ›</div>
                  </div>
                )}
                {warehouseSummary.lowStockItems > 0 && (
                  <div style={{ flex: 1, background: '#fff7ed', border: '1px solid #ffedd5', borderRadius: 12, padding: 15, display: 'flex', alignItems: 'center', gap: 12, cursor: 'pointer' }} onClick={() => router.push('/dashboard/peralatan/gudang')}>
                    <div style={{ background: '#f59e0b', color: '#fff', width: 40, height: 40, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20 }}>⚠️</div>
                    <div>
                      <div style={{ fontWeight: 'bold', color: '#9a3412' }}>{warehouseSummary.lowStockItems} Stok Menipis</div>
                      <div style={{ fontSize: 12, color: '#d97706' }}>Segera lakukan pengadaan</div>
                    </div>
                    <div style={{ marginLeft: 'auto', color: '#f59e0b', fontWeight: 'bold' }}>Lihat ›</div>
                  </div>
                )}
              </div>
            )}
