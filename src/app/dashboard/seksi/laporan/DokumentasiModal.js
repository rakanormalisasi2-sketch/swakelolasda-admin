...
export default function DokumentasiModal({ logs, onClose, onPrint, pdfConfig, handleUploadTambahan }) {
  const [selectedLog, setSelectedLog]     = useState(null); // log object
  const [dokSelection, setDokSelection]   = useState({}); // { [logId_idx]: '0%'|'50%'|'100%'|'skip' }
  const [filter, setFilter]               = useState('all'); // 'all'|'selected'|'unselected'
  const [search, setSearch]               = useState('');

  // Build flat list of rows with photo count
  const rows = useMemo(() => {
    return logs.map(log => {
  ...
  const setAllProgress = (logId, urls, val) =>
    setDokSelection(p => {
      const n = { ...p };
      urls.forEach((_, i) => { n[logId + '_' + i] = val; });
      return n;
    });

  const execPrint = () => {
    const config = pdfConfig || { program: 'PENGELOLAAN SUMBER DAYA AIR', kegiatan: '', pekerjaanPrefix: 'NORMALISASI SUNGAI' };
    const SUB_MAP = {
    ...
    let html = `<html><head><title>Foto Dokumentasi</title>
      <style>
        ...
        * { margin:0; padding:0; box-sizing:border-box; }
        ...
        .photo-wrapper { border: 1px solid #000; margin-bottom: 12px; page-break-inside: avoid; }
        .photo-row { display: flex; }
        .photo-cell { width: 62%; border-right: 1px solid #000; display: flex; align-items: center; justify-content: center; padding: 10px; min-height: 200px; }
        .photo-cell img { max-width: 100%; max-height: 200px; object-fit: contain; display: block; }
        .caption-cell { width: 38%; padding: 15px 12px; display: flex; flex-direction: column; justify-content: center; }
        .caption-label { font-weight: bold; font-size: 11px; margin-bottom: 4px; }
        .progress-badge { display: inline-block; padding: 3px 10px; border-radius: 12px; font-weight: bold; font-size: 11px; margin-top: 6px; }
    ...
    logs.forEach(log => {
      ...
      const selected = urls.map((url, i) => {
        const prog = dokSelection[log.id + '_' + i];
        return prog && prog !== 'skip' ? { url, prog } : null;
      }).filter(Boolean).sort((a, b) => {
        const o = { '0%': 0, '50%': 1, '100%': 2 };
        return (o[a.prog] || 0) - (o[b.prog] || 0);
      });
      ...
      const pekerjaan = (SUB_MAP[log.assignment?.job_sub_type] || config.pekerjaanPrefix);
      const tgl = new Date(log.tanggal).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' }).toUpperCase();

      html += `<div class="page">
        <div class="doc-title">FOTO DOKUMENTASI</div>
        <table class="info-table">
          <tr><td>PROGRAM</td><td>:</td><td>${config.program || ''}</td></tr>
          <tr><td>KEGIATAN</td><td>:</td><td>${config.kegiatan || ''}</td></tr>
          <tr><td>PEKERJAAN</td><td>:</td><td>${pekerjaan} DESA ${desa} KECAMATAN ${kec}</td></tr>
      ...
      selected.forEach(item => {
        ...
        html += `<div class="photo-wrapper">
          <div class="photo-row">
            <div class="photo-cell"><img src="${toImg(item.url)}" /></div>
            <div class="caption-cell">
              <div class="caption-label">KETERANGAN :</div>
              <div style="font-size:11px;margin-top:4px;">FOTO PROSES PELAKSANAAN</div>
              <div class="progress-badge ${cls}">${item.prog === '0%' ? 'PROGRESS 0%' : item.prog === '50%' ? 'PROGRESS 50%' : 'PROGRESS 100%'}</div>
            </div>
          </div>
        </div>`;
      });
      ...
      <div style={{ width:'96vw', height:'94vh', background:'#fff', borderRadius:16, display:'flex', flexDirection:'column', overflow:'hidden', boxShadow:'0 25px 50px rgba(0,0,0,0.4)' }}>
        ...
        <div style={{ background:'linear-gradient(135deg,#1e3a5f,#2563eb)', color:'#fff', padding:'16px 24px', display:'flex', alignItems:'center', justifyContent:'space-between', flexShrink:0 }}>
          <div style={{ display:'flex', alignItems:'center', gap:12 }}>
            ...
            <div>
              ...
              <div style={{ fontSize:12, opacity:0.8 }}>Pilih foto & tandai progress sebelum mencetak PDF</div>
            </div>
          </div>
          <div style={{ display:'flex', alignItems:'center', gap:12 }}>
            {totalSelected > 0 && (
          ...
          <div style={{ flex:1, display:'flex', flexDirection:'column', overflow:'hidden' }}>
            {!selectedLog ? (
              <div style={{ flex:1, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', color:'#94a3b8' }}>
                ...
                <div style={{ fontSize:13, marginTop:6 }}>untuk melihat dan menandai foto</div>
              </div>
            ) : (
              <>
                {/* log info bar */}
                  ...
                  {selectedUrls.length === 0 ? (
                    ...
                    <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(190px, 1fr))', gap:16 }}>
                      {selectedUrls.map((url, uIdx) => {
                        ...
                        return (
                          <div key={uIdx} style={{
                            borderRadius:12, overflow:'hidden', border: `2px solid ${isTagged ? PROGRESS_COLOR[val] : '#e2e8f0'}`,
                            background:'#fff', boxShadow: isTagged ? `0 4px 12px ${PROGRESS_COLOR[val]}33` : '0 2px 6px rgba(0,0,0,0.06)',
                      ...
                      <label htmlFor={'up_dok_'+selectedLog.id}
                        style={{ borderRadius:12, border:'2px dashed #93c5fd', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', height:220, cursor:'pointer', background:'#eff6ff', transition:'all 0.2s' }}>
                        <input type="file" id={'up_dok_'+selectedLog.id} multiple accept="image/*" style={{ display:'none' }} onChange={(e) => handleUploadTambahan(e, selectedLog.id, 'lapangan')} />
                        <span style={{ fontSize:32 }}>📤</span>
                        <span style={{ fontSize:12, fontWeight:700, color:'#1d4ed8', marginTop:8, textAlign:'center' }}>Upload Foto Susulan</span>
        ...
        <div style={{ padding:'14px 24px', borderTop:'1px solid #e2e8f0', background:'#fff', display:'flex', alignItems:'center', justifyContent:'space-between', flexShrink:0 }}>
          <div style={{ fontSize:13, color:'#475569' }}>
            {totalSelected === 0
              ? '💡 Pilih baris lalu tandai foto dengan progress untuk mengaktifkan tombol cetak'
              : <span><span style={{ fontWeight:700, color:'#2563eb' }}>{totalSelected} foto</span> dari {logs.filter(l=>l.foto_lapangan_urls).length} laporan siap dicetak</span>
            }
          </div>
          <div style={{ display:'flex', gap:10 }}>
            ...
            <button onClick={execPrint} disabled={totalSelected === 0}
              style={{ padding:'9px 22px', borderRadius:8, border:'none', cursor: totalSelected===0 ? 'not-allowed' : 'pointer', fontWeight:700, fontSize:13,
                background: totalSelected===0 ? '#e2e8f0' : 'linear-gradient(135deg,#1e3a5f,#2563eb)', color: totalSelected===0 ? '#94a3b8' : '#fff',
                boxShadow: totalSelected===0 ? 'none' : '0 4px 12px rgba(37,99,235,0.4)', transition:'all 0.2s' }}>
              🖨️ Cetak PDF ({totalSelected} foto)
            </button>
          </div>
        </div>
      </div>
