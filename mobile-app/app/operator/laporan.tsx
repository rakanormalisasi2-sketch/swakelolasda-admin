...
export default function LaporanScreen() {
  ...
  const [keteranganKategori, setKeteranganKategori] = useState<'Kerusakan'|'Cuaca'|'Lainnya'|'Grease'|''>('');
  const [kateOpen, setKateOpen] = useState(false);
  const KATEGORI_OPTIONS = ['Kerusakan', 'Cuaca', 'Lainnya', 'Grease'];

  // Warehouse stock state for Grease validation
  const [greaseStock, setGreaseStock] = useState<number | null>(null);
  ...
  useEffect(() => {
    ...
    // Load grease stock from warehouse API
    loadGreaseStock();
  }, []);

  // Load grease stock from warehouse system (DB2)
  const loadGreaseStock = async () => {
    try {
      // Fetch from web-admin warehouse API
      // Using hardcoded base URL - in production this would come from env
      const baseUrl = process.env.EXPO_PUBLIC_SUPABASE_URL?.replace('rest', 'api') || 'https://ratmptlcrjifuplokask.supabase.co';
      const res = await fetch(`${baseUrl}/functions/v1/gudang?type=items`, {
        headers: { 'apikey': process.env.EXPO_PUBLIC_SUPABASE_ANON_KEY || '' }
      });
      if (res.ok) {
        const data = await res.json();
        const greaseItem = data.data?.find((item: any) =>
          item.name?.toLowerCase().includes('grease') || item.name?.toLowerCase().includes('stempet')
        );
        if (greaseItem) {
          setGreaseStock(greaseItem.current_stock || 0);
        } else {
          setGreaseStock(0); // No grease item found - disable option
        }
      }
    } catch (e) {
      console.warn('Could not load grease stock:', e);
      setGreaseStock(0); // Default to 0 if API unavailable
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // 3. Jika kategori Grease → kirim request ke warehouse
      if (keteranganKategori === 'Grease' && greaseQty && parseInt(greaseQty) > 0) {
        try {
          // Find grease item ID first
          const baseUrl = process.env.EXPO_PUBLIC_SUPABASE_URL?.replace('rest', 'api') || 'https://ratmptlcrjifuplokask.supabase.co';
          const itemsRes = await fetch(`${baseUrl}/functions/v1/gudang?type=items`, {
            headers: { 'apikey': process.env.EXPO_PUBLIC_SUPABASE_ANON_KEY || '' }
          });
          if (itemsRes.ok) {
            const itemsData = await itemsRes.json();
            const greaseItem = itemsData.data?.find((item: any) =>
              item.name?.toLowerCase().includes('grease') || item.name?.toLowerCase().includes('stempet')
            );
            if (greaseItem) {
              await fetch(`${baseUrl}/functions/v1/gudang`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', 'apikey': process.env.EXPO_PUBLIC_SUPABASE_ANON_KEY || '' },
                body: JSON.stringify({
                  action: 'create_request',
                  request_type: 'operator_grease',
                  item_id: greaseItem.id,
                  requested_qty: parseInt(greaseQty),
                  requested_by: operatorName,
                  requester_role: 'operator',
                  equipment_id: assignment?.equipment_id,
                  equipment_name: form.jenisAlat,
                })
              });
            }
          }
        } catch (e) {
          console.warn('Gagal kirim request grease ke gudang:', e);
          // Don't fail the whole submission if grease request fails
        }
      }
    ...
    <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
      <ScrollView style={styles.scroll} contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled">
        ...
        <View style={styles.fieldGroup}>
          ...
          {kateOpen && (
            <View style={styles.dropdownBox}>
              ...
              {KATEGORI_OPTIONS.map(opt => {
                const isGreaseDisabled = opt === 'Grease' && (greaseStock === null || greaseStock <= 0);
                const isDisabled = opt === 'Kerusakan' ? false : isGreaseDisabled;
                return (
                  <TouchableOpacity
                    key={opt}
                    style={[styles.dropdownItem, isDisabled && { opacity: 0.5, backgroundColor: '#f7fafc' }]}
                    onPress={() => {
                      if (isDisabled) return;
                      setKeteranganKategori(opt as any);
                      setKateOpen(false);
                      if (opt === 'Grease') setGreaseQty('');
                    }}
                    disabled={isDisabled}
                  >
                    <Text style={[
                      styles.dropdownText,
                      keteranganKategori === opt && styles.dropdownActive,
                      opt === 'Kerusakan' && { color: '#e53e3e', fontWeight: '700' },
                      opt === 'Grease' && { color: '#dd6b20' },
                      isGreaseDisabled && { color: '#a0aec0' }
                    ]}>
                      {opt === 'Kerusakan' ? '🔴 Kerusakan Alat' :
                       opt === 'Cuaca' ? '🌧️ Cuaca' :
                       opt === 'Grease' ? `🛢️ Grease (Stok: ${greaseStock ?? '...'})` :
                       '📝 Lainnya'}
                      {opt === 'Grease' && greaseStock !== null && greaseStock <= 0 && ' ❌ Kosong'}
                    </Text>
                  </TouchableOpacity>
                );
              })}
          ...
          {keteranganKategori !== '' && (
            <>
              {keteranganKategori === 'Grease' ? (
                <View style={{ marginTop: 8 }}>
                  <View style={{ padding: 12, backgroundColor: '#fffaf0', borderRadius: 8, marginBottom: 8, borderWidth: 1, borderColor: '#dd6b20' }}>
                    <Text style={{ color: '#92400e', fontSize: 12, fontWeight: '600' }}>
                      🛢️ Permintaan Grease dari Gudang
                    </Text>
                    <Text style={{ color: '#92400e', fontSize: 11, marginTop: 2 }}>
                      Stok tersedia: {greaseStock ?? '...'} Pail
                    </Text>
                  </View>
                  <TextInput
                    style={styles.input}
                    ...
                    onChangeText={v => {
                      ...
                      if (v === '' || (!isNaN(num) && num > 0 && num <= (greaseStock ?? 0))) {
                        setGreaseQty(v.replace(/[^0-9]/g, ''));
                      } else if (v === '') {
                        setGreaseQty('');
                      }
                    }}
                    placeholder={`Jumlah Pail (maks: ${greaseStock ?? '...'})`}
                    keyboardType="number-pad"
                    maxLength={3}
                  />
                </View>
